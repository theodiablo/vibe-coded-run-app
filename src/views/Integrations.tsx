...
export function Integrations({ settings, saveSettings, showToast, scanImportsNow }: IntegrationsProps) {
  ...
  const connect = async (p: ImportProvider) => {
    ...
    try {
      ...
      if (ok) {
        ...
        scanImportsNow?.().then(n => { if (!n) showToast?.("No new runs to import yet."); }).catch(() => {});
      } else {
        showToast?.("Access wasn't granted. Allow Running Coach to read Exercise, Distance, Elevation and Heart rate.", "err");
      }
    } catch {
      showToast?.("Couldn't connect. Make sure Health Connect is installed and up to date.", "err");
    } finally {
      setBusyId(null);
    }
  };
  ...
  const scanOlder = async () => {
    ...
    try {
      ...
      showToast?.("Couldn't read your connected sources.", "err");
    } finally {
      setScanning(false);
    }
  };
  ...
  return (
    <div className="space-y-3 pt-2 border-t border-slate-700/60">
      ...
      <p className="text-xs text-slate-500 -mt-1">
        ...
        watch, so you don&apos;t have to carry your phone.
      </p>
            ...
            {on ? (
              <>
                <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-3 py-2 text-sm text-emerald-300">
                  ...
                  <span>{p.label} — new runs are offered for import automatically.</span>
                </div>
