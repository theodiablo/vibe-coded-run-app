...
export function RunRow({run, dateFmt = fmt.sht, showNotes = false, actions = null}) {
  const pace = run.km && run.durationSec ? run.durationSec / run.km : 0;
  return (
    <div className="bg-slate-800 rounded-xl p-3 flex items-center gap-3">
      <div className={"w-1.5 h-10 rounded-full flex-shrink-0 " + runBarColor(run.type)}/>
      ...
      <span className={"text-xs font-semibold flex-shrink-0 " + (TCLR[run.type] || TCLR.OTHER)}>{run.type}</span>
      {actions}
    </div>
  );
}
